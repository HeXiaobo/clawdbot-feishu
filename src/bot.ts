...
try:
    with pdfplumber.open('${filePath}') as pdf:
        ...
        print(text[:50000])  # Limit to 50k chars
except Exception as e:
    print(f'Error: {e}', file=sys.stderr)
    sys.exit(1)
`;
    return execSync(`python3 -c "${script}"`, { encoding: "utf-8", maxBuffer: 10 * 1024 * 1024 });
  } catch (err) {
    throw new Error(`PDF extraction failed: ${err}`);
  }
}

// --- Helper: Extract text from Word using python-docx ---
...
// --- Helper: Extract text from Excel using openpyxl ---
...
try:
    ...
    output = []
    for sheet_name in wb.sheetnames[:3]:  # First 3 sheets only
        sheet = wb[sheet_name]
        output.append(f'\\n=== Sheet: {sheet_name} ===')
        for row in sheet.iter_rows(max_row=min(sheet.max_row, 100)):  # Max 100 rows
            row_text = ' | '.join([str(cell.value) if cell.value is not None else '' for cell in row])
            if row_text.strip():
                output.append(row_text)
    result = '\\n'.join(output)
    print(result[:50000])  # Limit to 50k chars
except Exception as e:
    print(f'Error: {e}', file=sys.stderr)
    sys.exit(1)
`;
